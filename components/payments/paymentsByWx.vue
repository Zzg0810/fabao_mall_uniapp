...
export default {
	...
	methods: {
		...
		formatPayments (payments) {
			// 过滤支付宝支付
			payments = payments.filter(item => item.code == 'wechatpay' || item.code == 'balancepay')
		...
		async toPayHandler (code) {
			...
			switch (code) {
				case 'wechatpay':
					...
					let res = await Api.apiCall('post',Api.order.miniAppPay,params);
					console.log(res);
					if (res) {
							...
							success: function (e) {
								if (e.errMsg === 'requestPayment:ok') {
									_this.$common.successToShow(res.msg, () => {
										_this.$common.redirectTo('/pages/order/payment/result?id=' + params.orderId)
									})
								}else{
