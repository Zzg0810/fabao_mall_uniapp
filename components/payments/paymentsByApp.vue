<template>
	<view class="pay-type-list">
		...
		<view class="type-item b-b" v-for="item in paymentss" :key="item.code" @click="toPayHandler(item.code)"
			...
			<text v-if=" item.code == 'wechatapppay'" class="icon yticon icon-weixinzhifu"></text>
...
export default {
	...
	methods: {
		...
		payments = payments.filter(item => item.code == 'alipay' || item.code == 'balancepay' || item.code == 'wechatapppay');
		...
		redirectHandler (paymentId) {
			this.$common.redirectTo('/pages/order/payment/result?id=' + this.orderId)
		}
	}
