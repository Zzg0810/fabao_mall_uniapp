  ...
  <div class="total-wrap" :style="{marginTop: isMainNode?'22rpx':'6rpx'}">
    <div class="node-container">
      ...
      <div class="node-container-right" :style="{paddingTop: isMainNode?'0':'8rpx'}">
        <div v-if="isMainNode" class="node-title" :style="{color: isNewest?'#222':'#999'}">{{nodeData.AcceptStation}}</div>
        <div class="node-desc" :style="{color: isNewest?'#4b4b4b':'#999', margin:'10rpx 0'}">{{statusFixed}}</div>
        <div v-if="nodeData.AcceptStation" class="node-phone">{{nodeData.AcceptStation}}</div>
...
<script>
// https://ext.dcloud.net.cn/plugin?id=1065
export default {
  props: {
    isNewest: {
      type: Boolean,
  ...
  .node-container {
    ...
    &-left {
      ...
      .line-container {
        ...
        align-items: center;
        justify-content: center;
		height:calc(100% - 88rpx) !important;
        .line {
          width: 2rpx;
          background-color: #dcdcdc;
        }
