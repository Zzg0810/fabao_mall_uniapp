<template>
	<view>
		<!-- 地址 -->
<!-- 		<view class="choicePicker">
			<text class="label">是否使用自提点</text>
			<radio-group @change="radioChange" class="radio-group">
				<label >
					<view>
						<radio value="1" :checked="isUse == 1" />
					</view>
					<view>是</view>
				</label>
				<label >
					<view>
						<radio value="0" :checked="isUse == 0" />
					</view>
					<view>否</view>
				</label>
			</radio-group>
		</view> -->
		<view class="pickerAddressList" v-if="isUse == 1">
			 <view class="address-item" v-for="(item, i) in pikeList" :key="i" :class="{active: choicePikeId == item.id}" @click="choicePike(item)">
				 <view>
					 自提点名:{{item.addressName}}
				 </view>
				 <view>
					 自提地址:{{item.province}} {{item.city}} {{item.region}} {{item.detailAddress}}
				 </view>
				 <view>
					 联系人:{{item.name}}
				 </view>
		...
		<navigator url="../../pagesU/address/address?source=1" class="address-section" v-if="isUse == 0">
		...
		<view class="goods-section" v-for="(item1, index1) in confirmOrderResultList" :key="index1">
			<view class="g-header b-b">
				...
				<text class="name"> 共{{ item1.cartPromotionItemList.length }}件法宝</text>
			...
			<view class="g-item" v-for="(item, index) in item1.cartPromotionItemList" :key="index">
				...
				<view class="right">
					...
					<view class="price-box">
						<text class="price">{{ item.weight }}g</text>
...
<!-- 			<view class="yt-list">
			...
			<view class="yt-list">
				<view class="yt-list-cell b-b">
					<text class="cell-tit clamp">法宝重量</text>
					<text class="cell-tip">{{allWeight }}g</text>
				</view>

				<!-- <view class="yt-list-cell b-b" v-if="item1.calcAmount.promotionAmount>0">
					<text class="cell-tit clamp">优惠金额</text>
					<text class="cell-tip red">-{{unit}}{{ item1.calcAmount.promotionAmount }}</text>
				</view> -->

				<view class="yt-list-cell b-b">
					<text class="cell-tit clamp">运费</text>
					<text class="cell-tip">{{unit}}{{freightAll}}</text>
				</view>
				<view class="yt-list-cell b-b">
					<text class="cell-tip">说明：大件另參照物流費用, 若因經濟困難，無法承擔到付費用，請聯絡敝會，酌情處理。</text>
				</view>
			...
			<view class="mask-content" @click.stop.prevent="stopPrevent">
				...
				<view class="coupon-item" v-if="couponList.length > 0" v-for="(item, index) in couponList" :key="index">
					<view class="con" @click="selectCoupon(item)">
						...
						<view class="right">
							<text class="price">{{unit}} {{ item.coupon.amount }}</text>
...
<script>
	...
	export default {
		...
		computed: {
			...
			...mapGetters(['unit']),
			allWeight () {
				const {confirmOrderResultList} = this
				let weight = 0
				if (confirmOrderResultList.length) {
					confirmOrderResultList.forEach(r => {
						if (r.cartPromotionItemList && r.cartPromotionItemList.length) {
							r.cartPromotionItemList.forEach(rr => {
								weight+= parseInt(rr.weight || 0) * rr.quantity
							})
						}
					})
				}
				console.log(confirmOrderResultList)
				return parseInt(weight) || 0
			},
			freightAll () {
				 const {freight, isUse} = this
				 return isUse == 0 ? freight : 0
			}
		},
		filters: {
		...
		methods: {
			...
			async submit() {
				console.log('dubmit')
				let data;
				const {isUse, choicePikeId} = this
				if (!this.addressId) {
					this.$api.msg('请选择收货地址');
					return;
				}
				if (isUse ==1) {
					if (!this.choicePikeId) {
						this.$api.msg('请选择自提点');
						return
					}
				}
				...
				// } else {
					...
					if (this.type == 2) {
						...
						params = {
							...
							addressId: this.addressId,
							couponId: this.couponId,
							storeData: JSON.stringify(goods_list1),
							payType: 7,
							source: 0,
							pickupAddressId: this.isUse == 1 ? this.choicePikeId : '0',
							freightAmount:this.freightAll,
							content: ''
						...
						// let url = '/pages/order/payment/index';  // 跳到付款页面
						let url = '/pages/order/order?status=0';
						this.$loading(false)
						if (data && data.order) {
							// url = url+ '?order_id='+data.order.id + 'type=1'
							uni.navigateTo({
								url: url
							});
						}
			...
			stopPrevent() {},
			async getPikeAddressList () {
				this.$loading()
				const data = await Api.apiCall('get', Api.goods.getPickupAdressList, {pickupSide:0});
				this.pikeList = data || []
				this.$loading(false)
			},
			radioChange (e) {
				this.isUse = e.detail.value
				if (this.isUse == 0) {
					this.choicePikeId  = ''
				}
			},
			...
			async getFreight () {
				let param = {}
				const {allWeight, addressData} = this
				param = {
					weight: allWeight,
					memberAddressId: addressData.id
				}
				const data = await  Api.apiCall('get', Api.order.getFreightprice, param)
				this.freight = data || 0
			},
			choicePike (item) {
				this.choicePikeId = this.choicePikeId == item.id ? '' : item.id
			}
		}
	};
</script>
