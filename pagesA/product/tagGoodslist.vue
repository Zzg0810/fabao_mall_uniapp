<template>
	<view class="content">
		<view class="navbar" :style="{ position: headerPosition, top: headerTop }">
			<view class="nav-item" :class="{ current: filterIndex === 0 }" @click="tabClick(0)">綜合排序</view>
			<view class="nav-item" :class="{ current: filterIndex === 1 }" @click="tabClick(1)">銷量優先</view>
		...
		<view class="goods-list" v-show="current === 0">
			<view v-for="(item, index) in goodsList" :key="index" class="goods-item" @click="navToDetailPage(item)">
				...
				<view class="price-box" v-if="item.isFenxiao == 1">
					...
					<text>傭金 ¥{{ item.fenxiaoPrice }}</text>
				</view>
				<view class="price-box" v-if="item.isVip == 1">
					<text class="price">會員價{{ item.vipPrice }}</text>
					<text>法寶價 ¥{{ item.price }}</text>
				</view>
