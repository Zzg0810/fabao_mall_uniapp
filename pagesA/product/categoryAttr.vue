<template>
	<view class="content" style="display: flex;flex-direction: column;">
		...
		<view class="search" @click="search()">
			<input class="search_input" type="text" disabled="true" placeholder="輸入關鍵字搜索" />
		</view>
		<!-- #endif -->
...
export default {
	...
	data() {
		return {
			...
			right_height: [], //右側每個內容的高度集合
			right_scroll: 0, //右側的滑動值
			left_height: 0, //左側總高度
			left_scroll: 0, //左側滑動值
			last: 0,
	...
	methods: {
		...
		choose(index) {
			...
			// 加入防抖
			if (this.timeout) {
				clearTimeout(this.timeout); //清除計時器
			}
		...
		myscroll(e) {
			...
			let scroll_top = e.detail.scrollTop + 110; //110是banner圖的高度
			//判斷當前的scrollTop在哪個區間內;
			let now = +new Date();
